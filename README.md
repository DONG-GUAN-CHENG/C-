...
***
#### ***串列(List)***  
當程式需要儲存同一類型的資料時，陣列是一個簡單的方法，而串列的出現是為了解決一些陣列的缺陷，串列的觀念是根據執行時所需記憶體大小，動態配置一塊記憶體，稱之為節點(node)，節點之間包含了連結的機制，將所有node串接在一起，稱之為串列
```
串列的優點:  
1. 程式執行時可動態配置記憶體，所使用的空間可以自由增減，減少記憶體的浪費  
2. 當元素需要被插入或刪除時，不影響其他空間，以增進效率  
```
```
串列的缺點:  
1. 製作不易，需使用指標完成資料結構  
...
為最基本的串列，每個節點包含了:儲存的資料、指向下一個節點的指標  
```
struct node{
int data;
struct node *next;
