...
ps 1:使用malloc函式向作業系統要求一塊大小等同於int型態的記憶體，使用sizeof時，可以避免不同作業系統的int型態大小不的狀態，回傳時先將型態轉換為指向int的指標型態，在指定記憶體位址給p，接下來就可利用指標p間接存取這塊記憶體。而當使用者使用結構時，結構大小並不一定等同成員大小的總和，因使在使用malloc()配置記憶體時，最好不要直接指定位元組，而應使用sizeof 運算子找出需要的大小      
